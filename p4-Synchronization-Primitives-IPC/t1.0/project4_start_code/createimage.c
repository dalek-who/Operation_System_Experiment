...
{   
    ...
    int size;
    int size_kernel;
    int kernel_sector_num = count_sectors(kernel_ehdr,kernel_phdr,&size_kernel);
    int sectors_num = kernel_sector_num;
    size = size_kernel;

    printf("size of kernel:%d\n",size_kernel);
    printf("sector number of kernel:%d\n",kernel_sector_num);
    ...
    fwrite(buf,1,0x7200,*image_file);
    
    //把三个process写进内核。如果少于三个，文件指针会返回NULL
    char buf_proc[0xe00]="";
    int max_proc=6;
    char process_name[6][20] = {"process1","process2","process3","processes","process5","process6"};
    FILE *p_process[max_proc];
    Elf32_Ehdr* ehdr_process[max_proc];
    Elf32_Phdr* phdr_process[max_proc];
    int sector_num_process[max_proc],
        size_process[max_proc];
    
    int j=0;
    for(i=0;i<max_proc;++i){
        memset(buf_proc,0,0xe00);
        phdr_process[i] = read_exec_file(&p_process[i], process_name[i], &ehdr_process[i]);
        if(p_process[i] == NULL){
            printf("********************no %s\n",process_name[i]);
            continue;
        } 
       // sector_num_process[i] = count_sectors(ehdr_process[i],&size_process[i]);
      //  size += size_process[i];
     //   printf("size of process %d:%d\n",size_process[i],i+1);
     //   printf("sector number of process %d:%d\n",sector_num_process[i],i+1);

        for(j=0,already_read =0; j<ehdr_process[i]->e_phnum ; ++j){
            fseek(p_process[i],phdr_process[i][j].p_offset,SEEK_SET);
            fread(buf_proc+already_read,phdr_process[i][j].p_filesz,1,p_process[i]);
            already_read += phdr_process[i][j].p_filesz;
        }
        fwrite(buf_proc,1,0xe00,*image_file);
        printf("**********************write %s\n",process_name[i]);
        fclose(p_process[i]);
    }
