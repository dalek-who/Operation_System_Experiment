...
int count_kernel_sectors(Elf32_Ehdr *kernel_header, Elf32_Phdr *kernel_phdr,int *size){
	int sectors_num;
	int more;
	//todo
    int i;
...
int main(int argc, char *argv[]){
	...
	int size;
	int num_sec = count_kernel_sectors(kernel_ehdr,kernel_phdr,&size);
    //通过改机器码，改变image里bootblock的kernel size参数值
	record_kernel_sectors(&image,kernel_ehdr, kernel_phdr, num_sec);
