...
//int weight_table[3]={5,1,8};
int weight_setting[]={2,5,1,4,5,6,7,8,9,10};
int weight_table[NUM_TASKS];
int scheduler_table[NUM_TASKS];
...
void __attribute__((section(".entry_function"))) _start(void)
{
     ...
     printstr("wang kernel weight-666\n");
     ...
     //give weight
     for(i=0; i < NUM_TASKS; ++i){
        weight_table[i]=weight_setting[i];
        scheduler_table[i] = (i==0)? weight_table[i] : scheduler_table[i-1] + weight_table[i];
     }
     ...
     {
        colome+=5;
        printf(2,colome,"%d ",weight_table[i]);
     }
     
     printf(3,50,"scheduler: ");
     colome=70;
     for (i = 0; i < NUM_TASKS; ++i)
     {
        colome+=5;
        printf(3,colome,"%d ",scheduler_table[i]);
     }
